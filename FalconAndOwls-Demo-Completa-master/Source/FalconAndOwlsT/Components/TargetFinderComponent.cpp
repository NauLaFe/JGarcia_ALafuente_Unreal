...
void UTargetFinderComponent::FindTargets()
{
	...
	// que tipo de objetos tienes que detectar
...
#endif
	// hacer un shape tracing																con forma de caja
	...
	{
		UClass *hittedActorClass = hit.GetActor()->GetClass();
		if (!TargetTypes.Contains(hittedActorClass)) continue;
		...
		if (hit.GetActor() != CurrentTarget)
		{
			...
			// si el item es mayor a (-1) es que ha colisionado con un objeto multimalla
			if (hit.Item < 0)
			{
				currentTargetLocation = hit.GetComponent()->GetComponentLocation();
			}
